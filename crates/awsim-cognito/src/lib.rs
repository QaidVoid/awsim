...
impl ServiceHandler for CognitoService {
    ...
    async fn handle(
        ...
        let ctx = ctx.clone();
        run_blocking(move || dispatch(state, &operation, input, &ctx)).await
    }
...
/// Route one Cognito operation to its handler.
///
/// Every operation here is synchronous, and several are CPU-heavy:
/// bcrypt password verification, SRP modular exponentiation, and up to
/// three RS256 signatures per successful sign-in. [`CognitoService::handle`]
/// runs this on tokio's blocking pool so a burst of sign-ins cannot
/// occupy the runtime's worker threads and stall every other service
/// sharing the process.
fn dispatch(
    state: Arc<CognitoState>,
    operation: &str,
    input: Value,
    ctx: &RequestContext,
) -> Result<Value, AwsError> {
    match operation {
        ...
        // User Pool Clients
        "CreateUserPoolClient" => operations::pools::create_user_pool_client(&state, &input, ctx),
        "DescribeUserPoolClient" => {
            operations::pools::describe_user_pool_client(&state, &input, ctx)
        }
        "DeleteUserPoolClient" => operations::pools::delete_user_pool_client(&state, &input, ctx),
        ...
        "AdminSetUserPassword" => operations::users::admin_set_user_password(&state, &input, ctx),
        ...
        "ConfirmForgotPassword" => operations::users::confirm_forgot_password(&state, &input, ctx),
        ...
        // MFA configuration
        "SetUserPoolMfaConfig" => operations::mfa::set_user_pool_mfa_config(&state, &input, ctx),
        "GetUserPoolMfaConfig" => operations::mfa::get_user_pool_mfa_config(&state, &input, ctx),
        "AssociateSoftwareToken" => operations::mfa::associate_software_token(&state, &input, ctx),
        ...
        "UpdateUserPoolClient" => operations::pools::update_user_pool_client(&state, &input, ctx),
        ...
        "AdminAddUserToGroup" => operations::groups::admin_add_user_to_group(&state, &input, ctx),
        "AdminRemoveUserFromGroup" => {
        ...
        "UpdateUserAttributes" => operations::users::update_user_attributes(&state, &input, ctx),
        "DeleteUserAttributes" => operations::users::delete_user_attributes(&state, &input, ctx),
        ...
        "GetIdentityProviderByIdentifier" => {
            operations::identity_providers::get_identity_provider_by_identifier(&state, &input, ctx)
        }

        // Domain management
        "CreateUserPoolDomain" => operations::domain::create_user_pool_domain(&state, &input, ctx),
        "DescribeUserPoolDomain" => {
            operations::domain::describe_user_pool_domain(&state, &input, ctx)
        }
        "DeleteUserPoolDomain" => operations::domain::delete_user_pool_domain(&state, &input, ctx),
        ...
        // User import jobs
        "CreateUserImportJob" => operations::import::create_user_import_job(&state, &input, ctx),
        "DescribeUserImportJob" => {
        ...
        "UpdateUserPoolDomain" => operations::domain::update_user_pool_domain(&state, &input, ctx),

        // Pool-level misc
        "GetSigningCertificate" => operations::pools::get_signing_certificate(&state, &input, ctx),
        "GetLogDeliveryConfiguration" => {
...
/// Run a synchronous Cognito operation on tokio's blocking pool.
///
/// Mirrors the DynamoDB service's helper of the same name. Cheap when
/// the operation does no real work; the pool reuses its threads.
    ...
    F: FnOnce() -> Result<Value, AwsError> + Send + 'static,
{
    match tokio::task::spawn_blocking(f).await {
        Ok(res) => res,
        Err(join_err) => {
            tracing::warn!(error = %join_err, "Cognito blocking task panicked");
            Err(AwsError::internal(format!(
                "Cognito worker join error: {join_err}"
            )))
        }
    }
}
