...
pub struct RequestEvent {
    ...
    /// Billable read units consumed by the call, populated when the
    /// responding service sets the `X-Awsim-Read-Units` header.
    /// DynamoDB emits its consumed RCU here so the meter can charge
    /// per request unit (the AWS on-demand billing unit) instead of
    /// per API call. May be fractional (eventually consistent reads
    /// cost 0.5 RRU per 4 KiB).
    pub read_units: Option<f64>,
    /// Billable write units consumed by the call, from the
    /// `X-Awsim-Write-Units` header. Kept separate from `read_units`
    /// because AWS prices the two axes differently and a single call
    /// (PartiQL transactions) can consume both.
    pub write_units: Option<f64>,
}
