...
async fn test_ssm(endpoint: &str, verbose: bool) -> Vec<OpResult> {
    ...
    // CreateActivation
    let create_act_r = client
        .create_activation()
        .iam_role("SSMInstanceRole")
        .description("conformance activation")
        .send()
        .await;
    let activation_id = create_act_r
        .as_ref()
        .ok()
        .and_then(|r| r.activation_id.clone());
    results.push(chk!("CreateActivation", create_act_r, verbose));
    ...
    // CreateOpsMetadata
    let create_meta_r = client
        .create_ops_metadata()
        .resource_id("/conformance/opsmeta")
        .send()
        .await;
    let meta_arn = create_meta_r
        .as_ref()
        .ok()
        .and_then(|r| r.ops_metadata_arn.clone());
    results.push(chk!("CreateOpsMetadata", create_meta_r, verbose));
    ...
    // DeleteOpsMetadata
    if let Some(ref marn) = meta_arn {
        results.push(chk!(
            "DeleteOpsMetadata",
            client.delete_ops_metadata().ops_metadata_arn(marn).send().await,
            verbose
        ));
    } else {
    ...
    results.push(chk!(
        "DescribeAvailablePatches",
        client.describe_available_patches().send().await,
        verbose
    ));
    ...
    if let Some(ref cid) = command_id {
        results.push(chk!(
            "CancelCommand",
            client.cancel_command().command_id(cid).send().await,
            verbose
        ));
    } else {
        results.push(OpResult::Skipped("CancelCommand".to_string()));
    }

    // ListCommandInvocations
    results.push(chk!(
        "ListCommandInvocations",
        client.list_command_invocations().send().await,
        verbose
    ));
...
async fn test_cloudfront(endpoint: &str, verbose: bool) -> Vec<OpResult> {
    ...
    results.push(chk!(
        ...
        client.list_response_headers_policies().send().await,
        verbose
    ));
    ...
    let cp_r = client
        .create_cache_policy()
        .cache_policy_config(
            aws_sdk_cloudfront::types::CachePolicyConfig::builder()
                .name("conformance-cache-policy")
                .min_ttl(1)
                .default_ttl(86400)
                .max_ttl(31536000)
                .parameters_in_cache_key_and_forwarded_to_origin(
                    aws_sdk_cloudfront::types::ParametersInCacheKeyAndForwardedToOrigin::builder()
                        .enable_accept_encoding_gzip(true)
                        .headers_config(
                            aws_sdk_cloudfront::types::CachePolicyHeadersConfig::builder()
                                .header_behavior(
                                    aws_sdk_cloudfront::types::CachePolicyHeaderBehavior::None,
                                )
                                .build()
                                .unwrap(),
                        )
                        .cookies_config(
                            aws_sdk_cloudfront::types::CachePolicyCookiesConfig::builder()
                                .cookie_behavior(
                                    aws_sdk_cloudfront::types::CachePolicyCookieBehavior::None,
                                )
                                .build()
                                .unwrap(),
                        )
                        .query_strings_config(
                            aws_sdk_cloudfront::types::CachePolicyQueryStringsConfig::builder()
                                .query_string_behavior(
                                    aws_sdk_cloudfront::types::CachePolicyQueryStringBehavior::None,
                                )
                                .build()
                                .unwrap(),
                        )
    ...
    let cp_id = cp_r
        .as_ref()
        .ok()
        .and_then(|r| r.cache_policy.as_ref())
        .map(|p| p.id.clone());
    results.push(chk!("CreateCachePolicy", cp_r, verbose));

    if let Some(ref id) = cp_id {
        results.push(chk!(
            "GetCachePolicy",
            client.get_cache_policy().id(id).send().await,
            verbose
        ));
        results.push(chk!(
            "DeleteCachePolicy",
            client.delete_cache_policy().id(id).send().await,
            verbose
        ));
    } else {
        results.push(OpResult::Skipped("GetCachePolicy".to_string()));
        results.push(OpResult::Skipped("DeleteCachePolicy".to_string()));
    }
    ...
    let orp_r = client
        ...
        .origin_request_policy_config(
            aws_sdk_cloudfront::types::OriginRequestPolicyConfig::builder()
                .name("conformance-origin-request")
                .headers_config(
    ...
    let orp_id = orp_r
        ...
        .map(|p| p.id.clone());
    results.push(chk!("CreateOriginRequestPolicy", orp_r, verbose));
    ...
    if let Some(ref id) = orp_id {
        ...
        results.push(chk!(
            ...
            client.delete_origin_request_policy().id(id).send().await,
            verbose
        ));
    } else {
        results.push(OpResult::Skipped("GetOriginRequestPolicy".to_string()));
        results.push(OpResult::Skipped("DeleteOriginRequestPolicy".to_string()));
    }
    ...
    let pk_id = pk_r
        ...
        .and_then(|p| p.id.clone());
    results.push(chk!("CreatePublicKey", pk_r, verbose));
    ...
    let kg_r = client
        ...
        .key_group_config(
            aws_sdk_cloudfront::types::KeyGroupConfig::builder()
                .name("conformance-key-group")
                .items(pk_id.clone().unwrap_or_else(|| "K1".to_string()))
                .build()
                .unwrap(),
        )
        .send()
    ...
    results.push(chk!("CreateKeyGroup", kg_r, verbose));
    ...
    if let Some(ref id) = kg_id {
        ...
        results.push(chk!(
            "DeleteKeyGroup",
            client.delete_key_group().id(id).send().await,
            verbose
        ));
    } else {
        results.push(OpResult::Skipped("GetKeyGroup".to_string()));
        results.push(OpResult::Skipped("DeleteKeyGroup".to_string()));
    }

    if let Some(ref id) = pk_id {
        results.push(chk!(
            "DeletePublicKey",
            client.delete_public_key().id(id).send().await,
            verbose
        ));
    } else {
        results.push(OpResult::Skipped("DeletePublicKey".to_string()));
    }

    let fle_r = client
        .create_field_level_encryption_config()
        .field_level_encryption_config(
            aws_sdk_cloudfront::types::FieldLevelEncryptionConfig::builder()
                .caller_reference("conformance-fle-ref")
                .comment("conformance")
                .build()
                .unwrap(),
        )
    ...
    let fle_id = fle_r
        .as_ref()
        .ok()
        .and_then(|r| r.field_level_encryption.as_ref())
        .and_then(|f| f.id.clone());
    results.push(chk!("CreateFieldLevelEncryptionConfig", fle_r, verbose));

    if let Some(ref id) = fle_id {
        results.push(chk!(
            "GetFieldLevelEncryptionConfig",
            client.get_field_level_encryption_config().id(id).send().await,
            verbose
        ));
        results.push(chk!(
        ...
        results.push(OpResult::Skipped("GetFieldLevelEncryptionConfig".to_string()));
        results.push(OpResult::Skipped("DeleteFieldLevelEncryptionConfig".to_string()));
    }
    ...
    let fn_r = client
        ...
        .send()
        .await;
    results.push(chk!("CreateFunction", fn_r, verbose));

    results.push(chk!(
        "DescribeFunction",
        client.describe_function().name("conformanceFn").send().await,
        verbose
    ));
...
async fn test_elb(endpoint: &str, verbose: bool) -> Vec<OpResult> {
    ...
    if let Some(ref arn) = lb_arn {
        ...
        results.push(OpResult::Skipped("ModifyLoadBalancerAttributes".to_string()));
        results.push(OpResult::Skipped("SetSecurityGroups".to_string()));
        results.push(OpResult::Skipped("SetSubnets".to_string()));
    }

    // ModifyTargetGroupAttributes / DescribeTargetHealth / Register/Deregister
    if let Some(ref arn) = tg_arn {
        results.push(chk!(
        ...
        results.push(chk!(
            ...
            client.describe_target_health().target_group_arn(arn).send().await,
            verbose
        ));
        results.push(chk!(
            "DeregisterTargets",
        ...
        results.push(OpResult::Skipped("ModifyTargetGroupAttributes".to_string()));
        results.push(OpResult::Skipped("RegisterTargets".to_string()));
        results.push(OpResult::Skipped("DescribeTargetHealth".to_string()));
        results.push(OpResult::Skipped("DeregisterTargets".to_string()));
    }

    // ModifyListener + DescribeListenerCertificates (need listener arn)
    if let Some(ref l_arn) = listener_arn {
        results.push(chk!(
            "ModifyListener",
            client
        ...
        let rule_r = client
            ...
            .actions(
                aws_sdk_elasticloadbalancingv2::types::Action::builder()
                    ...
                    .target_group_arn(tg_arn.clone().unwrap_or_default())
                    .build(),
            )
            .send()
            .await;
        let rule_arn = rule_r
            ...
            .and_then(|r| r.rule_arn.clone());
        results.push(chk!("CreateRule", rule_r, verbose));
        ...
        if let Some(ref r_arn) = rule_arn {
            results.push(chk!(
                "ModifyRule",
                client.modify_rule().rule_arn(r_arn).send().await,
                verbose
            ));
            results.push(chk!(
                "DeleteRule",
        ...
        results.push(OpResult::Skipped("ModifyListener".to_string()));
        results.push(OpResult::Skipped("DescribeListenerCertificates".to_string()));
...
async fn test_glue(endpoint: &str, verbose: bool) -> Vec<OpResult> {
    ...
    results.push(chk!(
        "SearchTables",
        client.search_tables().search_text("conformance").send().await,
        verbose
    ));
    ...
    results.push(chk!(
        "StartCrawler",
        client.start_crawler().name("conformance-crawler").send().await,
        verbose
    ));
    results.push(chk!(
        "StopCrawler",
        client.stop_crawler().name("conformance-crawler").send().await,
        verbose
    ));
    results.push(chk!(
        "UpdateCrawler",
    ...
    results.push(chk!(
        "GetCrawlerMetrics",
        client.get_crawler_metrics().send().await,
        verbose
    ));
    results.push(chk!(
        "GetClassifiers",
        client.get_classifiers().send().await,
        verbose
    ));
    results.push(chk!(
        "GetClassifier",
        client.get_classifier().name("nonexistent").send().await,
        verbose
    ));
    ...
    let jr_r = client
        ...
        .send()
        .await;
    let run_id = jr_r.as_ref().ok().and_then(|r| r.job_run_id.clone());
    results.push(chk!("StartJobRun", jr_r, verbose));
    if let Some(ref rid) = run_id {
        ...
        results.push(OpResult::Skipped("GetJobRun".to_string()));
        results.push(OpResult::Skipped("BatchStopJobRun".to_string()));
    }
    results.push(chk!(
        "GetJobRuns",
        client
    ...
    results.push(chk!(
        "GetConnections",
        client.get_connections().send().await,
        verbose
    ));
    results.push(chk!(
        "DeleteConnection",
        client
            .delete_connection()
            .connection_name("conformance-conn")
            .send()
            .await,
        verbose
    ));

    let res_arn = "arn:aws:glue:us-east-1:000000000000:database/conformance_db";
    results.push(chk!(
        "TagResource",
        client
            .tag_resource()
            .resource_arn(res_arn)
            .tags_to_add("env", "conformance")
            .send()
            .await,
        verbose
    ));
    results.push(chk!(
        "GetTags",
        client.get_tags().resource_arn(res_arn).send().await,
        verbose
    ));
    results.push(chk!(
        "UntagResource",
    ...
    results.push(chk!(
        ...
        client
            .create_script()
            .language(aws_sdk_glue::types::Language::Python)
            .send()
            .await,
        verbose
    ));
    results.push(chk!(
        ...
        client.get_catalog_import_status().send().await,
        verbose
    ));
...
async fn test_athena(endpoint: &str, verbose: bool) -> Vec<OpResult> {
    ...
    // GetNamedQuery + BatchGetNamedQuery
    if let Some(ref nqid) = named_query_id {
        ...
        results.push(OpResult::Skipped("BatchGetNamedQuery".to_string()));
    }
    ...
    // StopQueryExecution + GetQueryResults + BatchGetQueryExecution
    if let Some(ref qid) = query_execution_id {
        results.push(chk!(
            "StopQueryExecution",
            client.stop_query_execution().query_execution_id(qid).send().await,
            verbose
        ));
        results.push(chk!(
            "GetQueryResults",
            client.get_query_results().query_execution_id(qid).send().await,
            verbose
        ));
        results.push(chk!(
        ...
        results.push(OpResult::Skipped("StopQueryExecution".to_string()));
        results.push(OpResult::Skipped("GetQueryResults".to_string()));
        results.push(OpResult::Skipped("BatchGetQueryExecution".to_string()));
    }
    ...
    results.push(chk!(
        ...
        client
            ...
            .statement_name("conformance-stmt")
            .work_group("conformance-workgroup")
    ...
    results.push(chk!(
        ...
        client
            ...
            .r#type(aws_sdk_athena::types::DataCatalogType::Lambda)
            .send()
            .await,
        verbose
    ));
