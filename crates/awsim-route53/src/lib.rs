...
impl ServiceHandler for Route53Service {
    ...
    fn routes(&self) -> Vec<RouteDefinition> {
        vec![
            ...
            // Record Sets. Route53 models the change path with a
            // trailing slash and the list path without one; the router
            // retries without it, so the bare form covers both.
            RouteDefinition {
                method: "POST",
