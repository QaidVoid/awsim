...
- `ListObjectsV2` supports `Prefix` filtering and `Delimiter` for virtual folder grouping (`CommonPrefixes`). The `KeyCount` response field reflects only object keys, not `CommonPrefixes` entries.
...
- **DeleteObjects** returns per-key error entries (with `Code` and `Message`) for any keys that fail to delete, matching AWS behavior. It does not silently swallow errors.
- **HeadObject** and **GetObject** return user metadata headers (`x-amz-meta-*`) that were supplied during `PutObject`.
- **Bucket/Object ACLs** use the caller's actual account ID as the canonical owner ID (not a placeholder string).
