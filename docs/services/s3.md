...
- Object Lock is a create-time property, as it is on real S3. Pass `--object-lock-enabled-for-bucket` to `CreateBucket` to turn it on, which also enables versioning; there is no API to enable it afterwards. `PutObjectLockConfiguration` on a bucket that was created without it is rejected with `InvalidBucketState`, and versioning cannot be suspended while it is on.
