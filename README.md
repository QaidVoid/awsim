...
- **Lambda execution.** AWSim runs your function code for real, executing `nodejs*` and `python*` handlers as local subprocesses via the host's `node` / `python3`. Other runtimes return `UnsupportedRuntime`. Supports event source mappings from SQS, Kinesis, and DynamoDB Streams. See [docs/guide/lambda-execution.md](docs/guide/lambda-execution.md).
- **Persistence.** `--data-dir` enables snapshot-based recovery for tables, queues, secrets, IAM, and most other services. Object/message/code/layer bodies are written to per-service body stores; orphan GC sweeps them on startup. Coverage is not yet universal: some services have no snapshot support, and a snapshot load reports exactly what it did and did not restore rather than claiming success. See [docs/guide/persistence.md](docs/guide/persistence.md).
