 ...
 *
 * In a packaged build the admin UI is served by the emulator itself, so
 * its own origin is always the right endpoint. Hardcoding the default
 * port meant every page failed with "Failed to fetch" whenever the
 * server ran anywhere else, which is the normal case when 4566 is
 * already taken.
 *
 * Under `bun run dev` the origin is Vite's, which serves the app at `/`
 * and so cannot also proxy `/` to the emulator. Dev therefore targets
 * the default port directly, which is where `awsim` listens unless it
 * was told otherwise.
 *
 * The last branch also covers prerendering, where there is no window;
 * the module re-evaluates in the browser during hydration.
 */
function resolveEndpoint(): string {
  if (import.meta.env.DEV) return "http://localhost:4566";
...
export function authHeader(service: string): string {
...
function parseXmlListBuckets(xml: string): { buckets: S3Bucket[] } {
  ...
  const regex =
    /<Name>([^<]+)<\/Name>\s*<CreationDate>([^<]+)<\/CreationDate>/g;
  let match;
...
export async function createQueue(
  ...
  const params: Record<string, unknown> = {
    QueueName: fifo ? (name.endsWith(".fifo") ? name : `${name}.fifo`) : name,
  };
  if (fifo) {
...
export async function deleteQueue(queueUrl: string): Promise<void> {
  await awsRequest(
    "sqs",
    "DeleteQueue",
    { QueueUrl: queueUrl } as unknown as Record<string, string>,
    "json",
    "AmazonSQS",
  );
}
...
export async function sendMessage(
  ...
  await awsRequest(
    "sqs",
    "SendMessage",
    { QueueUrl: queueUrl, MessageBody: body } as unknown as Record<
      string,
      string
    >,
    "json",
...
export async function deleteMessage(
  ...
  await awsRequest(
    "sqs",
    "DeleteMessage",
    { QueueUrl: queueUrl, ReceiptHandle: receiptHandle } as unknown as Record<
      string,
      string
    >,
    "json",
...
export async function describeTable(name: string): Promise<DynamoTableDetail> {
  const data = (await awsRequest(
    "dynamodb",
    "DescribeTable",
    { TableName: name } as unknown as Record<string, string>,
    "json",
    "DynamoDB_20120810",
  )) as {
    Table?: {
...
export async function putItem(
  tableName: string,
  item: Record<string, DynamoAttributeValue>,
): Promise<void> {
  await awsRequest(
    "dynamodb",
    "PutItem",
...
export async function deleteItem(
  tableName: string,
  key: Record<string, DynamoAttributeValue>,
): Promise<void> {
  await awsRequest(
    "dynamodb",
    "DeleteItem",
...
export async function iamListGroupsForUser(
  userName: string,
...
export async function iamListUserPolicies(
  userName: string,
): Promise<{ policyNames: string[] }> {
...
export async function iamListAttachedUserPolicies(
  userName: string,
): Promise<{ policies: IamAttachedPolicy[] }> {
...
export async function iamListAccessKeys(
  userName: string,
): Promise<{ accessKeys: IamAccessKey[] }> {
...
export async function iamCreateAccessKey(
  userName: string,
...
export async function iamDeleteAccessKey(
  userName: string,
  accessKeyId: string,
): Promise<void> {
...
export async function iamListUserTags(
  userName: string,
): Promise<{ tags: IamTag[] }> {
...
export async function iamTagUser(
  userName: string,
  tags: IamTag[],
): Promise<void> {
...
export async function iamUntagUser(
  userName: string,
  tagKeys: string[],
): Promise<void> {
  const params: Record<string, string> = { UserName: userName };
...
export async function iamGetRole(roleName: string): Promise<
  IamRole & {
...
export async function iamListRolePolicies(
  roleName: string,
): Promise<{ policyNames: string[] }> {
...
export async function iamListAttachedRolePolicies(
  roleName: string,
): Promise<{ policies: IamAttachedPolicy[] }> {
...
export async function iamListRoleTags(
  roleName: string,
): Promise<{ tags: IamTag[] }> {
...
export async function iamUpdateAssumeRolePolicy(
  roleName: string,
...
export async function iamGetPolicy(arn: string): Promise<
  IamPolicy & {
...
export async function iamListPolicyVersions(
  arn: string,
): Promise<{ versions: IamPolicyVersion[] }> {
...
export async function iamGetPolicyVersion(
  arn: string,
  versionId: string,
...
export async function iamCreatePolicyVersion(
  arn: string,
  document: string,
  setAsDefault = true,
): Promise<void> {
...
export async function iamDeletePolicyVersion(
  arn: string,
  versionId: string,
): Promise<void> {
  await iamRequest("DeletePolicyVersion", {
    PolicyArn: arn,
    VersionId: versionId,
  });
}

export async function iamCreatePolicy(
  name: string,
  document: string,
  description?: string,
): Promise<void> {
...
export async function iamGetGroup(
  name: string,
...
export async function iamListAttachedGroupPolicies(
  groupName: string,
): Promise<{ policies: IamAttachedPolicy[] }> {
...
export async function iamAddUserToGroup(
  userName: string,
  groupName: string,
): Promise<void> {
...
export async function iamRemoveUserFromGroup(
  userName: string,
  groupName: string,
): Promise<void> {
...
export async function iamAttachRolePolicy(
  roleName: string,
  policyArn: string,
): Promise<void> {
...
export async function iamDetachRolePolicy(
  roleName: string,
  policyArn: string,
): Promise<void> {
...
export async function iamAttachUserPolicy(
  userName: string,
  policyArn: string,
): Promise<void> {
...
export async function iamDetachUserPolicy(
  userName: string,
  policyArn: string,
): Promise<void> {
...
export async function iamAttachGroupPolicy(
  groupName: string,
  policyArn: string,
): Promise<void> {
...
export async function iamDetachGroupPolicy(
  groupName: string,
  policyArn: string,
): Promise<void> {
...
export async function iamGetAccountPasswordPolicy(): Promise<IamPasswordPolicy> {
  ...
  return {
    minimumPasswordLength: parseInt(
      xmlValue(xml, "MinimumPasswordLength") || "8",
      10,
    ),
    requireSymbols: xmlValue(xml, "RequireSymbols") === "true",
    requireNumbers: xmlValue(xml, "RequireNumbers") === "true",
    requireUppercaseCharacters:
      xmlValue(xml, "RequireUppercaseCharacters") === "true",
    requireLowercaseCharacters:
      xmlValue(xml, "RequireLowercaseCharacters") === "true",
    allowUsersToChangePassword:
      xmlValue(xml, "AllowUsersToChangePassword") === "true",
    expirePasswords: xmlValue(xml, "ExpirePasswords") === "true",
    maxPasswordAge: parseInt(xmlValue(xml, "MaxPasswordAge") || "0", 10),
    passwordReusePrevention: parseInt(
      xmlValue(xml, "PasswordReusePrevention") || "0",
      10,
    ),
...
export async function iamUpdateAccountPasswordPolicy(
  policy: Partial<IamPasswordPolicy>,
): Promise<void> {
  const params: Record<string, string> = {};
  if (policy.minimumPasswordLength !== undefined)
    params["MinimumPasswordLength"] = String(policy.minimumPasswordLength);
  if (policy.requireSymbols !== undefined)
    params["RequireSymbols"] = String(policy.requireSymbols);
  if (policy.requireNumbers !== undefined)
    params["RequireNumbers"] = String(policy.requireNumbers);
  if (policy.requireUppercaseCharacters !== undefined)
    params["RequireUppercaseCharacters"] = String(
      policy.requireUppercaseCharacters,
    );
  if (policy.requireLowercaseCharacters !== undefined)
    params["RequireLowercaseCharacters"] = String(
      policy.requireLowercaseCharacters,
    );
  if (policy.allowUsersToChangePassword !== undefined)
    params["AllowUsersToChangePassword"] = String(
      policy.allowUsersToChangePassword,
    );
  if (policy.maxPasswordAge !== undefined)
    params["MaxPasswordAge"] = String(policy.maxPasswordAge);
  if (policy.passwordReusePrevention !== undefined)
    params["PasswordReusePrevention"] = String(policy.passwordReusePrevention);
...
export async function deleteFunction(name: string): Promise<void> {
  const res = await loggedFetch(
    ...
    "DeleteFunction",
    "DELETE",
    `${ENDPOINT}/2015-03-31/functions/${encodeURIComponent(name)}`,
    {
      method: "DELETE",
      headers: lambdaHeaders(),
    },
...
export async function listUserPools(): Promise<{
...
export async function listCognitoUsers(
  ...
  const data = (await cognitoRequest("ListUsers", {
...
export async function createUserPoolDomain(
  userPoolId: string,
  domain: string,
): Promise<void> {
  await cognitoRequest("CreateUserPoolDomain", {
    UserPoolId: userPoolId,
    Domain: domain,
  });
}

export async function deleteUserPoolDomain(
  userPoolId: string,
  domain: string,
): Promise<void> {
  await cognitoRequest("DeleteUserPoolDomain", {
    UserPoolId: userPoolId,
    Domain: domain,
  });
}
...
export async function listResourceServers(
  userPoolId: string,
): Promise<{ servers: CognitoResourceServer[] }> {
  const data = (await cognitoRequest("ListResourceServers", {
    UserPoolId: userPoolId,
    MaxResults: 50,
  })) as {
    ResourceServers?: {
...
export async function deleteResourceServer(
  ...
  await cognitoRequest("DeleteResourceServer", {
    UserPoolId: userPoolId,
    Identifier: identifier,
  });
}
...
export async function listIdentityProviders(
  userPoolId: string,
): Promise<{ providers: CognitoIdentityProvider[] }> {
  const data = (await cognitoRequest("ListIdentityProviders", {
    UserPoolId: userPoolId,
    MaxResults: 60,
  })) as {
    Providers?: {
...
export async function deleteIdentityProvider(
  ...
  await cognitoRequest("DeleteIdentityProvider", {
    UserPoolId: userPoolId,
    ProviderName: providerName,
  });
}
...
export async function listUserPoolClients(
  userPoolId: string,
): Promise<{ clients: CognitoUserPoolClient[] }> {
  const data = (await cognitoRequest("ListUserPoolClients", {
    UserPoolId: userPoolId,
    MaxResults: 60,
  })) as {
    UserPoolClients?: {
...
export async function createUserPoolClient(
  ...
  if (opts?.generateSecret !== undefined)
    body["GenerateSecret"] = opts.generateSecret;
...
export async function describeUserPoolClient(
  userPoolId: string,
  clientId: string,
): Promise<CognitoUserPoolClientDetail> {
  const data = (await cognitoRequest("DescribeUserPoolClient", {
    UserPoolId: userPoolId,
    ClientId: clientId,
  })) as {
    UserPoolClient?: {
...
export async function deleteUserPoolClient(
  ...
  await cognitoRequest("DeleteUserPoolClient", {
    UserPoolId: userPoolId,
    ClientId: clientId,
  });
}
...
export async function adminCreateUser(
  ...
  if (opts?.email)
    body["UserAttributes"] = [{ Name: "email", Value: opts.email }];
...
export async function adminGetUser(
  ...
  const data = (await cognitoRequest("AdminGetUser", {
    UserPoolId: userPoolId,
    Username: username,
  })) as {
    Username?: string;
...
export async function adminSetUserPassword(
  ...
  await cognitoRequest("AdminSetUserPassword", {
    UserPoolId: userPoolId,
    Username: username,
    Password: password,
    Permanent: permanent,
...
export async function adminUpdateUserAttributes(
  ...
  await cognitoRequest("AdminUpdateUserAttributes", {
    UserPoolId: userPoolId,
    Username: username,
...
export async function listCognitoUsersWithAttrs(
  userPoolId: string,
): Promise<{ users: CognitoUser[] }> {
...
export async function listCognitoGroups(
  ...
  const data = (await cognitoRequest("ListGroups", {
    UserPoolId: userPoolId,
  })) as {
    Groups?: {
      GroupName: string;
...
export async function deleteCognitoGroup(
  ...
  await cognitoRequest("DeleteGroup", {
    UserPoolId: userPoolId,
    GroupName: name,
  });
}
...
export async function listUsersInGroup(
  ...
  const data = (await cognitoRequest("ListUsersInGroup", {
    UserPoolId: userPoolId,
    GroupName: groupName,
  })) as {
    Users?: {
...
export async function adminListGroupsForUser(
  ...
  const data = (await cognitoRequest("AdminListGroupsForUser", {
    UserPoolId: userPoolId,
    Username: username,
  })) as {
    Groups?: {
...
async function cognitoIdentityRequest(
  action: string,
  body: unknown,
): Promise<unknown> {
...
export async function listIdentityPools(): Promise<{
  identityPools: CognitoIdentityPool[];
}> {
  const data = (await cognitoIdentityRequest("ListIdentityPools", {
    MaxResults: 60,
  })) as {
    IdentityPools?: {
...
export async function setIdentityPoolRoles(
  poolId: string,
...
export async function cognitoGetId(
  identityPoolId: string,
...
export async function cognitoGetCredentials(
  identityId: string,
  logins?: Record<string, string>,
): Promise<unknown> {
...
export async function describeKey(keyId: string): Promise<KmsKeyDetail> {
  const data = (await awsRequest(
    "kms",
    "DescribeKey",
    { KeyId: keyId } as unknown as Record<string, string>,
    "json",
    "TrentService",
  )) as {
    KeyMetadata?: {
...
export async function listParameters(): Promise<{
...
export async function listEmailIdentities(): Promise<{
...
export async function deleteEmailIdentity(
  ...
  const res = await loggedFetch(
    ...
    "DeleteEmailIdentity",
    "DELETE",
    `${ENDPOINT}/v2/email/identities/${encodeURIComponent(emailIdentity)}`,
    {
      method: "DELETE",
      headers: sesHeaders(),
    },
...
export async function deleteEmailTemplate(templateName: string): Promise<void> {
  const res = await loggedFetch(
    ...
    "DeleteEmailTemplate",
    "DELETE",
    `${ENDPOINT}/v2/email/templates/${encodeURIComponent(templateName)}`,
    {
      method: "DELETE",
      headers: sesHeaders(),
    },
...
export async function listExecutions(
  stateMachineArn: string,
): Promise<{ executions: SfnExecution[] }> {
  const data = (await awsRequest(
    "states",
    "ListExecutions",
    {
...
export async function describeStateMachine(
  arn: string,
): Promise<{ definition: string; name: string; type: string; status: string }> {
  const data = (await awsRequest(
    "states",
    "DescribeStateMachine",
    { stateMachineArn: arn } as unknown as Record<string, string>,
    "json",
    "AWSStepFunctions",
  )) as {
    definition?: string;
...
export async function getExecutionHistory(
  arn: string,
): Promise<{ events: SfnHistoryEvent[] }> {
  const data = (await awsRequest(
    "states",
    "GetExecutionHistory",
    { executionArn: arn } as unknown as Record<string, string>,
    "json",
    "AWSStepFunctions",
  )) as {
    events?: {
...
export async function describeExecution(arn: string): Promise<{
  status: string;
  input?: string;
  output?: string;
  startDate: number;
...
export async function describeSecurityGroups(): Promise<{
...
export async function createSecurityGroup(
  ...
  await ec2Request("CreateSecurityGroup", {
    GroupName: name,
    Description: desc,
    VpcId: vpcId,
  });
...
export async function describeStackResources(
  stackName: string,
): Promise<{ resources: CfStackResource[] }> {
...
export async function describeStream(
  name: string,
): Promise<{ stream: KinesisStream; shards: KinesisShard[] }> {
  const data = (await awsRequest(
    "kinesis",
    "DescribeStream",
    {
...
export async function assumeRole(
  ...
  const xml = await stsRequest("AssumeRole", {
    RoleArn: roleArn,
...
export async function deleteDbInstance(id: string): Promise<string> {
  return rdsRequest("DeleteDBInstance", {
    DBInstanceIdentifier: id,
...
export async function describeLoadBalancers(): Promise<{
  loadBalancers: ElbLoadBalancer[];
}> {
...
export async function createLoadBalancer(
  ...
  await elbRequest("CreateLoadBalancer", {
    Name: name,
    Type: type,
    Scheme: scheme,
  });
...
export async function describeTargetGroups(): Promise<{
...
export async function describeListeners(
  loadBalancerArn: string,
): Promise<{ listeners: ElbListener[] }> {
  const xml = await elbRequest("DescribeListeners", {
