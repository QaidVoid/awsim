...
- **Model Gateway:** New `/gateway` page (top-level sidebar entry under
  AI/ML) hosts the entire Bedrock-proxy configuration surface that
  previously lived inside the Settings page. The Settings Bedrock
  section is gone; the `/bedrock` proxy tab keeps a thin read-only
  summary that links out.
- **gateway:** Bundled LLM provider catalog of 15 entries (Ollama,
  LM Studio, vLLM, llama.cpp, LocalAI, OpenAI, Anthropic, Groq,
  Mistral, Together, Fireworks, OpenRouter, DeepInfra, Bedrock,
  Custom) served at `GET /_awsim/gateway/catalog`. Powers the
  Add-Backend wizard with endpoint templates, auth-field hints,
  env-var placeholders, and a curated model list per provider.
- **bedrock:** `[credentials]` table on `BedrockSpec`. A single
  api_key (inline or via env var) can be referenced from multiple
  `[backends.*]` blocks via `credential = "<name>"`.
- **bedrock:** Per-backend `provider` field on `BackendSpec` (pure
  UI metadata that records the catalog key the backend was wired
  from).
- **bedrock:** `[aliases]` table keyed by Bedrock id with a chat
  or embed kind, a strategy (first), and an ordered list of
  `(backend, tag)` targets. The resolver checks aliases before
  the legacy `[invoke]` / `[embed]` tables; under the First
  strategy it walks targets in declaration order and returns the
  first whose backend is configured and not currently marked Down.
- **bedrock:** Automatic fallback across alias targets on
  retriable upstream errors (HTTP 5xx, 408, 429, reqwest network
  errors). Build-time errors and other 4xx stay fatal because
  retrying would just route the same bad request to the next
  backend.
- **bedrock:** Per-target request overrides on alias targets
  (`timeout_ms`, `max_tokens`, `temperature`). Timeout works on
  both chat and embed paths; the other two apply to chat only.
- **bedrock:** Backend health registry + a background poller that
  pings each configured backend's `/models` every 30 seconds.
  Sticky transitions (two failures flip to Down, one success
  flips back to Healthy). The alias resolver skips Down targets
  so a failed primary recovers without the request having to
  fail first.
- **bedrock:** In-process metrics registry (per `(bedrock_id,
  backend)` counters split by outcome plus a 12-bucket latency
  histogram with p50/p95) and a 200-entry recent-invocations
  ring. Both reset on restart.
- **gateway admin endpoints:** `GET /_awsim/gateway/catalog`,
  `GET /_awsim/gateway/health`,
  `POST /_awsim/gateway/health/{name}/check`,
  `GET /_awsim/gateway/metrics`, `GET /_awsim/gateway/recent`,
  `POST /_awsim/gateway/test-prompt`.
- **ui/gateway:** Six sub-tabs - Backends (catalog browser +
  Add/Edit wizard with provider templates and credential
  picker), Credentials (CRUD with Used-by chips), Models &
  Aliases (unified view of aliases + legacy mappings with a
  multi-target editor, per-target override panel, drag-to-
  reorder, inline test-prompt panel, and live call/p50/p95
  chips), Activity (last-200 invocations with per-attempt
  expand), Health (status pills + 30-tick sparkline + Recheck),
  Playground (links to existing Bedrock playground for now).
- **ui/bedrock:** Status dots inline on each Backends row showing
  Healthy / Degraded / Down / Unknown at a glance.

### Compatibility

- Legacy `[invoke]` / `[embed]` entries in existing TOML configs
  and `runtime-config.json` keep loading and routing unchanged.
  The runtime prefers aliases when both shapes exist for the
  same Bedrock id.
